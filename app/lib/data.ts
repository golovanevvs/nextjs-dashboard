...
export async function fetchRevenue() {
  try {
    ...
    const data: QueryResult<Revenue> = await client.query(
      `SELECT * FROM revenue;`
    );
...
export async function fetchFilteredInvoices(
  ...
  try {
    ...
    `
    const invoices: QueryResult<InvoicesTable> = await client.query(str,[query])

    // const invoices: QueryResult<InvoicesTable> = await client.query(
    //   `
    //   SELECT
    //     invoices.id,
    //     invoices.amount,
...
export async function fetchInvoiceById(id: string) {
  try {
    const data: QueryResult<InvoiceForm> = await client.query(
      `
      SELECT
        invoices.id,
